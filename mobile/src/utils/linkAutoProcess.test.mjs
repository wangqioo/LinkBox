...
import { getAutoProcessLinkUrl, isAutoProcessLinkText } from './linkAutoProcess.js'

test('isAutoProcessLinkText allows exact WeChat Zhihu and Bilibili video URLs', () => {
...
test('isAutoProcessLinkText extracts allowed share URLs from surrounding text', () => {
  assert.equal(
    getAutoProcessLinkUrl('【B站独家】标题 https://www.bilibili.com/video/BV1ZBjB6UEbt/?share_source=copy_web'),
...
test('isAutoProcessLinkText rejects generic URLs and mixed generic message text', () => {
  ...
  assert.equal(isAutoProcessLinkText('not a url'), false)
})
