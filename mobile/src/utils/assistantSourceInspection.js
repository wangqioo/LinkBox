export function formatAssistantScore(value) {
  if (typeof value !== 'number' || Number.isNaN(value)) return ''
  return value.toFixed(3)
}

export function formatAssistantHeadingPath(value) {
  if (Array.isArray(value)) return value.filter(Boolean).join(' > ')
  return typeof value === 'string' ? value : ''
}

export function assistantSourceInspectionRows(retrieval) {
  if (!retrieval) return []
  ...
  if (retrieval.rerank_mode) {
    ...
    rows.push({ label: '重排', value: rerankScore ? `${retrieval.rerank_mode} ${rerankScore}` : retrieval.rerank_mode })
  }
