...
Deployment targets must stay distinct:

Home server / primary production:

- 4060Ti x86_64 server reached at `ssh -p 6004 wq@150.158.146.192`.
- App path: `/home/wq/LinkBox`.
- Runtime: Docker Compose service `linkbox`.
- Public URL: `http://150.158.146.192:6057/`.
- Rollback path pattern: `/home/wq/LinkBox-prev-<timestamp>`.

Small-box local Agent node:
...
Do not describe the RK3576/NanoPi-R76S node as the home server. Deploy it only
when the work specifically affects the small-box local Agent/runtime or the user
asks for the small-box deployment too.
...
- Build the new release before stopping the live service.
- Back up the live release before switching.
- Preserve persistent database, uploads, certs, and environment files for the
  selected target.
