...
export default function SocialPage() {
  ...
  useEffect(() => {
    setLoading(true);
    loadSocial().catch(e => setError(e.message || '加载协作数据失败')).finally(() => setLoading(false));
  }, []);

  useEffect(() => {
    if (!activeGroupId) return;
    loadGroup(activeGroupId).catch(e => setError(e.message || '加载群数据失败'));
  }, [activeGroupId]);
  ...
  const askGroupAgent = async (e: FormEvent) => {
    ...
    try {
      ...
      setAssistantAnswer(err instanceof Error ? err.message : '群资料助理暂时无法回答。');
    } finally {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold">好友与群组</h1>
          <p className="text-sm text-gray-500">和好友建群、发群消息、共享重要资料，每个群都有独立资料助理。</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-4">
        <aside className="space-y-4">
          <section className="card p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="font-semibold">我的群</h2>
            </div>
            <div className="space-y-2">
              {groups.map(group => (
                <button key={group.id} type="button" onClick={() => setActiveGroupId(group.id)}
                  ...
                  <div className="text-xs text-gray-500 mt-1">{group.member_count} 人 · {group.material_count} 条资料</div>
                </button>
              ))}
              {!groups.length && <div className="text-sm text-gray-500">还没有群，先邀请好友创建一个。</div>}
            </div>
          </section>
          ...
          <section className="card p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="font-semibold">拉群</h2>
            </div>
            <form onSubmit={createGroup} className="space-y-3">
              <input value={groupName} onChange={e => setGroupName(e.target.value)} className="input w-full" placeholder="群名称" />
              <textarea value={groupDescription} onChange={e => setGroupDescription(e.target.value)} className="input w-full min-h-[72px]" placeholder="群说明" />
        ...
        <main className="space-y-4 min-w-0">
          {activeGroup ? (
            <>
              <section className="card p-4">
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 text-sm text-indigo-600 dark:text-indigo-400">
                    <Bot className="w-4 h-4" />{activeGroup.agent_name || '群资料助理'}
                  </div>
                </div>
              </section>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                <section className="card p-4 min-h-[420px] flex flex-col">
                  ...
                  <form onSubmit={sendMessage} className="mt-3 flex gap-2">
                    <input value={messageText} onChange={e => setMessageText(e.target.value)} className="input flex-1" placeholder="发一条群消息" />
                    <button className="btn-primary px-3" type="submit"><Send className="w-4 h-4" /></button>
                  </form>
                </section>

                <section className="card p-4 min-h-[420px] flex flex-col">
                  <div className="flex items-center gap-2 mb-3"><Bot className="w-4 h-4 text-indigo-600" /><h3 className="font-semibold">群资料助理</h3></div>
                  <form onSubmit={askGroupAgent} className="flex gap-2 mb-3">
                    <input value={assistantText} onChange={e => setAssistantText(e.target.value)} className="input flex-1" placeholder="问这个群共享资料里的问题" />
                  ...
                  <div className="flex-1 overflow-auto rounded-lg bg-gray-50 dark:bg-gray-800/60 p-3 text-sm">
                    {assistantAnswer ? <MarkdownRenderer content={assistantAnswer} /> : <span className="text-gray-500">群助理只检索这个群里共享过的资料。</span>}
                  </div>
                </section>
              </div>
              ...
              <section className="card p-4">
                ...
                <form onSubmit={shareMaterial} className="grid grid-cols-1 md:grid-cols-[1fr_1fr_auto] gap-2 mb-4">
                  ...
                  <input value={shareNote} onChange={e => setShareNote(e.target.value)} className="input" placeholder="分享说明" />
                  <button type="submit" className="btn-primary justify-center" disabled={!shareLinkId}>共享到群</button>
                </form>
            ...
            <section className="card p-8 text-center text-gray-500">创建或加入一个群后，就可以发群消息和整理群资料。</section>
          )}
        </main>
      </div>
    </div>
